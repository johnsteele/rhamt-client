...
import { IRhamtClient } from './main';
import * as os from 'os';
import * as EventBus from 'vertx3-eventbus-client';
import { RuntimeConfiguration, RhamtConfiguration, RhamtEvent } from 'rhamt-core';
...
export class RhamtClient implements IRhamtClient {
    ...
    private runConfiguration?: RhamtConfiguration;

    constructor(private runtimeConfiguration: RuntimeConfiguration) {
    }
    ...
    public analyze(config: RhamtConfiguration): Promise<String> {
        // TODO: Best way to make sure we no longer receive events 
        // from a previous analysis that's running.
        // TODO: Send a stop event to server, then on callback we start.        
        return new Promise<String>((resolve, reject) => {
            if (!this.isRunning()) {
                reject();
            }
    ...
    public terminate(): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            if (this.serverProcess && !this.serverProcess.killed) {
                ...
                this.bus!.close();
                this.runtimeConfiguration!.onStopped.emit(new RhamtEvent());
                resolve();
            }
            else {
            ...
            const outputListener = (data: string | Buffer) => {
                ...
                if (match) {
                    ...
                    this.bus.onopen = () => {
                        ...
                        connected = true;
                        this.isServerRunnig = true;
                        this.runtimeConfiguration.onStarted.emit(new RhamtEvent());
                        resolve();
                    };
    ...
    private spawn(): cp.ChildProcess {
        console.log('rhamt-client using ' + JSON.stringify(this.runtimeConfiguration));
        return cp.spawn(this.runtimeConfiguration.rhamtCli, 
            ["--startServer", String(this.runtimeConfiguration.port)], {cwd: os.homedir()});
    }
